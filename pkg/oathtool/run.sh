...
_usage() {
	printf "usage: %s check|build|pack <target> <oathtoolkit_version> [<options>]\n" "$0"
	printf "\ntarget:\n"
...
_check() {
	docker inspect --type=image "moo/static-oathtool:${_name}.${_target}" > /dev/null 2>&1
	return $?
}
...
case $_action in
	check) _check; exit $? ;;
	build)
		_prepare "build-oathtool.sh" oathtoolkit
		_build_docker "musl" \
			"moo/static-oathtool:${_name}.${_target}" \
			"${_arch}" "${_pkgs}" \
			"build-oathtool.sh" "${_ver}" "${_opt}"
		;;
	pack)
		! _check && _err "not built jet."
		cd -- "${_cdir}" || _err "cannot cd to ${_cdir}"
		_fname="oathtool-${_name}.${_target}"
		_tmp="${_cdir}/out/tmp"; rm -rf -- "${_tmp}"; mkdir -p -- "${_tmp}" || _err "mkdir ${_tmp}"
		echo "- dump ${_name}.${_target}"
		docker run --rm \
			-i "moo/static-oathtool:${_name}.${_target}" \
		...
		echo "- pack ${_fname}"
		strip --strip-all "${_tmp}/bin/oathtool" > /dev/null 2>&1
		cp "${_tmp}/bin/oathtool" "${_out}" || _err "cp"
		rm -f -- "${_out}.tar.gz"
